...
if ($_POST && isset($_POST)) {
	if ($_GET && isset($_GET['type'])) {
		if ($_GET['type'] == 'add') {
			if($zbp->Config('ydtu')->HasKey('homeSliderArray'))
			{
				$homeSliderArray = json_decode($zbp->Config('ydtu')->homeSliderArray,true);
			}
			...
			$zbp->Config('ydtu')->homeSliderArray = json_encode($homeSliderArray);
			$zbp->SaveConfig('ydtu');
			Redirect('slide.php');
